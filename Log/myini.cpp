...
//************************************************************************
vector<ININode>::size_type MyINI::SetValue(string root, string key, string value)
{
...
//************************************************************************
string MyINI::GetValue(string root, string key)
{
    map<string, SubNode>::iterator itr = map_ini.find(root);
    map<string, string>::iterator sub_itr = itr->second.sub_node.find(key);
    if ((sub_itr->second).empty())  return "";
    return sub_itr->second;
}
//上面GetValue函数代码存在bug,用于替代上面的代码
string MyINI::GetValue2(string root, string key)
{
#if 0
    for (map<string, SubNode>::iterator itr = this->map_ini.begin(); itr!= this->map_ini.end(); ++itr)
    {
        if(0 == root.compare(itr->first))
        {
            map<string, string>::iterator itr1=itr->second.sub_node.begin();
            for ( ; itr1!=itr->second.sub_node.end(); ++itr1)
            {
                cout << "    " << itr1->first << " = " << itr1->second << endl;
                if(0 == key.compare(itr1->first)) return itr1->second;
                return "";
            }
        }
    }
